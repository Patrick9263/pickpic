...
**The operator console (`/operator`, `GET /api/admin/operator/accounts`) is the one admin surface that is deliberately not account-scoped** — it lists every account with its users, storage and event/photo counts so beta-tester growth is answerable without a D1 console (#195). Its whole access control is `OPERATOR_EMAILS`, a secret holding the addresses allowed in (`wrangler secret put OPERATOR_EMAILS --env app`); unset means no operator exists and every operator route 403s, which is the intended state on the public and admin workers. A secret rather than a column because a privilege bit in the same database the console reads is editable by anything that can write that row, and because config is per-deployment — the public origin cannot even express an operator. `handleOperatorRequest` runs in `routeRequest` **before** the `AccountScope` is built and is the only admin-side handler that receives the bare `D1Database`; `handleAdminRequest` is typed against `TenantEnv` (`Omit<Env, "DB">`) and structurally cannot serve it. Keep it that way: the guard belongs at that one entry point, not spread into the scoped handlers. It is read-only by design, which is why there is no deliberate "operator mode" to enter — the day an operator route writes to somebody else's account is the day that stops being ceremony.
