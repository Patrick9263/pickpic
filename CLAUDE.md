...
- **Check the sandboxed `PATH`.** If `node`, `npm`, or `gh` is unavailable, load nvm (`source "$HOME/.nvm/nvm.sh"`) and run `nvm use` from the worktree to select `.nvmrc`'s Node 22; add `/opt/homebrew/bin` to `PATH` for `gh` if needed. Do it in the same call as the command; shell state does not persist between calls.
