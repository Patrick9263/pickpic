...
Session cookies and magic-link tokens live in `auth_sessions` / `auth_login_tokens` (migration 0015); only SHA-256 hashes are stored. The `/api/auth/*` routes exist **only where `AUTH_MODE` is `session`** and 404 everywhere else, so no *sign-in* link can be built from an origin that would not honour it.
