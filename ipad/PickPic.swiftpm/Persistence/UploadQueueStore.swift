...
final class UploadQueueStore: ObservableObject {
    ...
    /*
     * The implementation moved to UploadQueueRecovery so the launch
     * recovery walk could use it without dragging in this @MainActor
     * store. Forwarding rather than duplicating keeps one version of
     * the active-upload-time accounting.
     */
    private func stopActiveUploadTimer(
        _ progress: inout UploadProgress,
        at stoppedAt: Date
    ) {
        UploadQueueRecovery.stopActiveUploadTimer(
            &progress,
            at: stoppedAt
        )
    }
    ...
    private func load() {
        ...
        do {
            ...
            let recovery = UploadQueueRecovery.recover(
                jobs: decodedJobs,
                now: Date()
            )
            ...
            /*
             * Cancellation stays out here rather than inside the
             * transform: it reaches iPadOS through a singleton, and
             * keeping it on this side is what lets the recovery walk
             * be tested without BackgroundTasks. The coordinator only
             * touches BGTaskScheduler and its own handler table, so
             * running the cancels after the assignments above rather
             * than during the walk changes nothing observable.
             */
            for jobID in recovery
